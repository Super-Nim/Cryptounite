...
const Password = (props: any) => { 
    ...
    const onSubmitPassword = async (e: SyntheticEvent) => {
        e.preventDefault()
    
        const { data } = await axios.get('http://localhost:5000/customer/')
        const checkPassword = await data.filter( (obj: any) =>  // DEFINE PASSWORD OBJECT INTERFACE
        ...
        if (password !== confirmPassword) {
            ...
            setPasswordContains(false)
            setConfirmPassword('false') // WTF GOIN ON HERE CHECK CODE/RE-DO CODE
    ...
    return (
        ...
        <div className="passwordContainer">
            ...
            <form className="pass-form" onSubmit={onSubmitPassword}>
                <label className='pass-label' htmlFor="standard-adornment-password"> 
             </label>
                <input 
            ...
            <label className='pass-label' htmlFor="password"></label>
                <input 
                className="pass-input" 
                type="password" 
                name="password" 
